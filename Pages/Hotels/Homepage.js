...
class Homepage {
  ...
  nextButtonOnCalendarLocator =
    "(//button[@data-stid='date-picker-paging'])[2]";
  ...
  async selectDestinationFromAutoSuggestion(selectThis) {
    await this.commands.selectFromAutoSuggestions(
      this.destinationAutoSuggestionLocator,
      selectThis
    );
  }
  ...
  async selectCheckInDate(monthName, year, checkInDate) {
    const monthHeadingLocator =
      this.monthHeadingLocatorStarts + monthName + " " + year;
    const monthDatesLocator =
      this.monthDatesLocatorStarts +
      monthName +
      " " +
      year +
      this.monthDatesLocatorEnds;
    await this.commands.selectDateFromCalendar(
      monthHeadingLocator,
      this.nextButtonOnCalendarLocator,
      monthDatesLocator,
      checkInDate
    );
  }
